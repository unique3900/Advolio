          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 items-center gap-20 ">
              {
                  skills.map((item, index) => (
                      <motion.div initial={{y:index*10,opacity:0}} whileInView={{y:0,opacity:1}} viewport={{once:true}} transition={{delay:index *0.1}} className="flex flex-col gap-4 shadow-md justify-center items-center py-[120px] px-6 w-48 h-48  duration-75" key={item.id}>
